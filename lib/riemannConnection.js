...
var settings = { maxRetryDelay: 10000,
...
function connectToRiemann(config) {
    var reconnectMaximumRetries = config.reconnectMaximumRetries || settings.reconnectMaximumRetries,
        maxRetryDelay = config.maxRetryDelay || settings.maxRetryDelay;
    ...
    riemannClient.on('error', function (er) {
        ...
        reconnectTimeoutMS = reconnectTimeoutMS > maxRetryDelay ? maxRetryDelay : reconnectTimeoutMS;
